...
async def debug_morning_summary():
    ...
    logger.info("=== MORNING SUMMARY BUG INVESTIGATION ===")
    ...
    if result.total_meetings_equivalent == 0.0:
        logger.error("BUG CONFIRMED: total_meetings_equivalent is 0.0 when it should be > 0")
        if "completely free morning" in result.speech_text:
            logger.error("BUG CONFIRMED: Speech text says 'completely free morning'")
    else:
        logger.info("Bug NOT reproduced - meetings were counted correctly")
