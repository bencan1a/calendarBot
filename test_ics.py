...
def setup_test_logging(verbose: bool = False):
    ...
    return setup_logging(log_level=level, log_file=None)  # Console only for test mode
...
async def test_ics_configuration(url: Optional[str] = None, verbose: bool = False) -> bool:
    ...
    try:
        ...
        print(
            f"   - Health Status: {'✅ Healthy' if primary_health.get('healthy', False) else '❌ Unhealthy'}"
        )
            ...
            if events and verbose:
                ...
                for i, event in enumerate(events[:3]):  # Show first 3 events
                    ...
                    if event.body_preview:
                        desc = (
                            event.body_preview[:50] + "..."
                            if len(event.body_preview) > 50
        ...
        print(
            f"   - Overall Health: {'✅ Healthy' if summary.get('is_healthy', False) else '❌ Unhealthy'}"
        )
...
async def validate_ics_format(url: str, verbose: bool = False) -> bool:
    ...
    try:
        ...
        source = ICSSource(name="Test ICS Source", url=url, timeout=30)
...
Examples:
  ...
  python test_ics.py --validate-only --url URL          # Only validate format
    ...
    parser.add_argument("--url", help="ICS URL to test (overrides configuration)")
