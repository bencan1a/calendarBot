...
class TestEventRetrievalMethods:
    ...
    async def test_get_events_for_week_when_normal_flow_then_returns_events_by_date(self) -> None:
        ...
        controller._process_event_for_date_grouping = Mock(
            side_effect=lambda event, events_by_date: events_by_date[event.start_dt.date()].append(
                event
            )
        )
    ...
    def test_process_event_for_date_grouping_when_has_start_datetime_then_parses_and_adds(
        self,
    ) -> None:
