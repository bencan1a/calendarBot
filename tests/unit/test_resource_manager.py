...
class TestResourceManagerCSSLoading:
    ...
    def mock_registry_with_layouts(self):
        ...
        registry.get_layout_css_paths.return_value = [
            Path("/layouts/4x8/4x8.css"),
            Path("/layouts/4x8/common.css"),
        ]
        return registry
    ...
    def test_get_css_content_valid_layout(
        self, resource_manager, mock_registry_with_layouts
    ) -> None:
        """Test getting CSS content for valid layout."""
        mock_css_content = "body { margin: 0; } .calendar { display: grid; }"
        ...
        with patch("builtins.open", create=True) as mock_open:
            ...
            assert mock_css_content in result
            mock_registry_with_layouts.validate_layout.assert_called_with("4x8")
    ...
    def test_get_css_paths_for_layout(self, resource_manager, mock_registry_with_layouts) -> None:
        ...
        expected_paths = [Path("/layouts/4x8/4x8.css"), Path("/layouts/4x8/common.css")]
...
class TestResourceManagerJSLoading:
    ...
    def mock_registry_with_js(self):
        ...
        registry.get_layout_js_paths.return_value = [
            Path("/layouts/4x8/4x8.js"),
            Path("/layouts/4x8/utils.js"),
        ]
        return registry
    ...
    def test_get_js_content_valid_layout(self, resource_manager_js, mock_registry_with_js) -> None:
        ...
        mock_js_content = "function init() { console.log('Layout loaded'); }"

        with patch("builtins.open", create=True) as mock_open:
            mock_open.return_value.__enter__.return_value.read.return_value = mock_js_content

            result = resource_manager_js.get_js_content("4x8")

            assert mock_js_content in result
    ...
    def test_get_js_paths_for_layout(self, resource_manager_js, mock_registry_with_js) -> None:
        ...
        expected_paths = [Path("/layouts/4x8/4x8.js"), Path("/layouts/4x8/utils.js")]
...
class TestResourceManagerPathMethods:
    ...
    def mock_registry_with_layout_info(self, mock_layout_info):
        ...
        registry.layouts_dir = Path("/layouts")
        return registry
    ...
    def test_get_css_path_valid_layout_returns_first_css_file(
        ...
        expected_path = Path("/layouts/4x8/4x8.css")
    ...
    def test_get_js_path_valid_layout_returns_first_js_file(
        ...
        expected_path = Path("/layouts/4x8/4x8.js")
        assert result == expected_path
        mock_registry_with_layout_info.get_layout_with_fallback.assert_called_with("4x8")
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("4x8", Path("/layouts/4x8/4x8.css"), Path("/layouts/4x8/4x8.js")),
            ("3x4", Path("/layouts/3x4/3x4.css"), Path("/layouts/3x4/3x4.js")),
        ],
    )
...
class TestResourceManagerCombinedOperations:
    ...
    def mock_registry_combined(self):
        ...
        registry = Mock()
        registry.validate_layout.return_value = True
        registry.get_layout_css_paths.return_value = [Path("/layouts/test/style.css")]
        registry.get_layout_js_paths.return_value = [Path("/layouts/test/script.js")]
        return registry
    ...
    def test_get_all_resources_for_layout(
        self, resource_manager_combined, mock_registry_combined
    ) -> None:
        """Test getting all resources (CSS and JS) for a layout."""
        css_content = "body { margin: 0; }"
        js_content = "function init() {}"

        with patch("builtins.open", create=True) as mock_open:
            mock_file = Mock()
            mock_file.read.return_value = css_content
            mock_open.return_value.__enter__.return_value = mock_file

            css_result = resource_manager_combined.get_css_content("test")

            # Reset for JS content
            mock_file.read.return_value = js_content
            js_result = resource_manager_combined.get_js_content("test")

            assert css_content in css_result
            assert js_content in js_result
...
class TestResourceManagerErrorHandling:
    ...
    def test_handles_file_permission_errors(self) -> None:
        ...
        mock_registry.get_layout_css_paths.return_value = [Path("/protected/style.css")]
    ...
    def test_handles_corrupted_files(self) -> None:
        ...
        mock_registry = Mock()
        mock_registry.validate_layout.return_value = True
        mock_registry.get_layout_css_paths.return_value = [Path("/layouts/test/corrupted.css")]
...
class TestResourceManagerFileOperationErrors:
    ...
    def test_get_css_content_when_mixed_file_errors_then_processes_available_files(
        self, mock_registry_error_scenarios
    ) -> None:
        """Test CSS content loading processes available files when some fail."""
        # Setup multiple CSS files, some will fail
        mock_registry_error_scenarios.get_layout_css_paths.return_value = [
            Path("/layouts/test/good.css"),
            Path("/layouts/test/bad.css"),
            Path("/layouts/test/good2.css"),
        ]

        resource_manager = ResourceManager(mock_registry_error_scenarios)

        def mock_open_side_effect(file_path, *args, **kwargs):
            if "bad.css" in str(file_path):
                raise PermissionError("Permission denied")

            # Create a proper context manager mock
            mock_file = Mock()
            mock_content_manager = Mock()
            mock_content_manager.read.return_value = f"/* Content from {file_path} */"
            mock_file.__enter__ = Mock(return_value=mock_content_manager)
        ...
        with patch("builtins.open", side_effect=mock_open_side_effect):
            ...
            # Should contain content from accessible files only
            assert "good.css" in result
            assert "good2.css" in result
            assert "bad.css" not in result
            assert len(result) > 0

    def test_get_js_content_when_mixed_file_errors_then_processes_available_files(
        self, mock_registry_error_scenarios
    ) -> None:
        """Test JS content loading processes available files when some fail."""
        # Setup multiple JS files, some will fail
        mock_registry_error_scenarios.get_layout_js_paths.return_value = [
            Path("/layouts/test/good.js"),
            Path("/layouts/test/bad.js"),
            Path("/layouts/test/good2.js"),
        ]

        resource_manager = ResourceManager(mock_registry_error_scenarios)

        def mock_open_side_effect(file_path, *args, **kwargs):
            if "bad.js" in str(file_path):
        ...
        with patch("builtins.open", side_effect=mock_open_side_effect):
            result = resource_manager.get_js_content("test")

            # Should contain content from accessible files only
            assert "good.js" in result
            assert "good2.js" in result
    ...
    def test_validate_layout_resources_when_files_missing_then_reports_correctly(
        ...
        resource_manager_error_test.layout_registry.layouts_dir = Path("/layouts")
