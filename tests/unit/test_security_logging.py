...
class TestSecurityEventLogger:
    ...
    def test_initialization_without_settings(self):
        ...
        with patch("calendarbot.utils.logging.get_logger") as mock_get_logger:
            mock_logger = Mock()
    ...
    def test_log_event_exception_handling(self):
        """Test log_event exception handling."""
        with patch("calendarbot.utils.logging.get_logger") as mock_get_logger:
            mock_logger = Mock()
            mock_audit_logger = Mock()
            mock_get_logger.return_value = mock_logger
            ...
            mock_audit_logger.info.side_effect = Exception("Audit failure")
            ...
            assert "Failed to log security event" in error_call
