...
class TestSafeAsyncCall:
    ...
    @pytest.mark.asyncio
    async def test_logging_disabled(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_logging_enabled(self) -> None:
        """Test error logging is enabled by default."""
        mock_func = AsyncMock(side_effect=ValueError("error"))

        with patch("calendarbot.utils.helpers.logger") as mock_logger:
            result = await safe_async_call(mock_func, log_errors=True)
...
class TestSecureClearScreen:
    ...
    @patch("calendarbot.utils.helpers.subprocess.run")
    @patch("builtins.print")
    def test_unexpected_error_fallback(self, mock_print: Any, mock_run: Any) -> None:
