...
class TestSavePngEmulation:
    ...
    def test_save_png_emulation_when_called_with_int_cycle_then_saves_image(
        self, mock_path: MagicMock, mock_pil_image: MagicMock
    ) -> None:
        ...
        mock_dir.__truediv__.return_value = mock_output_path
...
class TestInitializeEpaperComponents:
    ...
    async def test_initialize_epaper_components_when_successful_then_returns_context_and_settings(
        ...
        # Setup
        with patch("calendarbot.cli.modes.epaper.apply_command_line_overrides") as mock_apply_cmd_overrides, \
             patch("calendarbot.cli.modes.epaper.apply_cli_overrides") as mock_apply_cli_overrides, \
             patch("calendarbot.cli.modes.epaper.apply_epaper_mode_overrides") as mock_apply_epaper_overrides, \
    ...
    async def test_initialize_epaper_components_when_initialization_fails_then_raises_error(
        ...
        # Setup
        with patch("calendarbot.cli.modes.epaper.apply_command_line_overrides"), \
...
class TestRunEpaperMainLoop:
    ...
    @pytest.mark.asyncio
    async def test_run_epaper_main_loop_when_hardware_available_then_updates_display(
        self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_run_epaper_main_loop_when_no_hardware_then_saves_png(
        self) -> None:
    ...
    async def test_run_epaper_main_loop_when_exception_occurs_then_handles_error(
        ...
        mock_context.app.cache_manager.get_todays_cached_events.side_effect = Exception("Test error")
        
        # Execute
        with patch("calendarbot.cli.modes.epaper._handle_render_error") as mock_handle_error, \
...
class TestRunEpaperMode:
    ...
    async def test_run_epaper_mode_when_exception_occurs_then_returns_one(
        ...
        # Setup
        with patch("calendarbot.cli.modes.epaper._initialize_epaper_components") as mock_init, \
