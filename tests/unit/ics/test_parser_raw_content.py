...
END:VCALENDAR"""
    ...
    def test_parse_ics_content_when_empty_content_then_captures_empty_raw_content(
        ...
        """Test that empty content is still captured as raw content."""
        ics_content = ""

        # This will likely fail parsing but should still capture raw content
        result = parser.parse_ics_content(ics_content)
    ...
    def test_parse_ics_content_when_size_error_then_no_raw_content_and_raises_error(
        ...
        result = parser.parse_ics_content(oversized_content)
...
SUMMARY:Large Event """
        ...
        # Add enough content to exceed warning but stay under max
...
END:VCALENDAR"""
        ...
        # Mock the size validation to raise an exception during capture
        with patch.object(parser, "_validate_ics_size", side_effect=Exception("Capture error")):
            with patch("calendarbot.ics.parser.logger") as mock_logger:
                result = parser.parse_ics_content(ics_content)
