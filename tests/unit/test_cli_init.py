...
class TestMainEntry:
    ...
    async def test_main_entry_configured_interactive_mode(self, mock_parser_args):
        ...
        with patch("calendarbot.cli.create_parser") as mock_create_parser, patch(
            "calendarbot.cli.check_configuration"
        ) as mock_check_config, patch("calendarbot.cli.run_interactive_mode") as mock_interactive:
            mock_parser = MagicMock()
            mock_parser.parse_args.return_value = mock_parser_args
            mock_create_parser.return_value = mock_parser
    ...
    async def test_main_entry_configured_web_mode_default(self, mock_parser_args):
        ...
        with patch("calendarbot.cli.create_parser") as mock_create_parser, patch(
            "calendarbot.cli.check_configuration"
        ) as mock_check_config, patch("calendarbot.cli.run_web_mode") as mock_web:
            mock_parser = MagicMock()
            mock_parser.parse_args.return_value = mock_parser_args
            mock_create_parser.return_value = mock_parser
            mock_check_config.return_value = (True, "/path/to/config.yaml")
    ...
    async def test_main_entry_mode_count_calculation(
        ...
        with patch("calendarbot.cli.create_parser") as mock_create_parser, patch(
            ...
            if expected_count > 1:
                ...
                if mode_args.get("test_mode", False):
                    with patch("calendarbot.cli.run_test_mode") as mock_mode:
                        mock_mode.return_value = 0
                        result = await main_entry()
                        assert result == 0
                elif mode_args.get("interactive", False):
                    with patch("calendarbot.cli.run_interactive_mode") as mock_mode:
                        mock_mode.return_value = 0
                        result = await main_entry()
                        assert result == 0
                else:
                    # Default to web mode or explicit web mode
                    with patch("calendarbot.cli.run_web_mode") as mock_mode:
                        mock_mode.return_value = 0
                        result = await main_entry()
                        assert result == 0
    ...
    async def test_main_entry_async_mode_functions(self, mock_parser_args):
        ...
        for mock_func_name, args_dict in async_test_cases:
            ...
            with patch("calendarbot.cli.create_parser") as mock_create_parser, patch(
                "calendarbot.cli.check_configuration"
            ) as mock_check_config, patch(f"calendarbot.cli.{mock_func_name}") as mock_func:
                mock_parser = MagicMock()
                mock_parser.parse_args.return_value = mock_parser_args
                mock_create_parser.return_value = mock_parser
                mock_check_config.return_value = (True, "/path/to/config.yaml")
...
class TestCliIntegration:
    ...
    async def test_main_entry_integration_flow(self):
        ...
        with patch("calendarbot.cli.create_parser") as mock_create_parser, patch(
            "calendarbot.cli.check_configuration"
        ) as mock_check_config, patch("calendarbot.cli.run_web_mode") as mock_web_mode:
