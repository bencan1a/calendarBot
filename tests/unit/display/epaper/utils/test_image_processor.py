...
class TestImageProcessor:
    ...
    def test_convert_to_display_format_when_bw_display_then_converts_correctly(
        self, mock_convert: MagicMock, mock_resize: MagicMock, 
        processor: ImageProcessor, mock_image: MagicMock, mock_capabilities: MagicMock
    ) -> None:
        ...
        mock_resize.assert_called_once_with(
            mock_image, mock_capabilities.width, mock_capabilities.height, maintain_aspect_ratio=True
        )
    ...
    def test_convert_to_display_format_when_error_occurs_then_raises_exception(
        self, mock_resize: MagicMock, processor: ImageProcessor, 
        mock_image: MagicMock, mock_capabilities: MagicMock
    ) -> None:
    ...
    def test_resize_for_display_when_maintain_aspect_ratio_then_resizes_correctly(
        ...
        # Call method
        result = processor.resize_for_display(mock_image, mock_capabilities, maintain_aspect_ratio=True)
    ...
    def test_resize_for_display_when_not_maintain_aspect_ratio_then_resizes_correctly(
        ...
        # Call method
        result = processor.resize_for_display(mock_image, mock_capabilities, maintain_aspect_ratio=False)
        
        # Verify results
        assert result is mock_resize.return_value
        mock_resize.assert_called_once_with(
            mock_image, mock_capabilities.width, mock_capabilities.height, maintain_aspect_ratio=False
        )
    ...
    def test_create_test_image_when_bw_display_then_creates_correctly(
        self, mock_create_pattern: MagicMock, processor: ImageProcessor, mock_capabilities: MagicMock
    ) -> None:
