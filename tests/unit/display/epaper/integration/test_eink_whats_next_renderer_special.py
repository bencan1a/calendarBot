...
class TestEInkWhatsNextRendererSpecial:
    ...
    def test_render_error_when_basic_message_then_returns_formatted_image(
        ...
        with patch(
            ...
            with patch(
                "calendarbot.display.epaper.integration.eink_whats_next_renderer.ImageDraw.Draw",
                ...
                with patch.object(real_draw, "text") as mock_text:
                    ...
                    assert any(error_message in str(call) for call in text_calls), (
                        "Error message should be drawn"
                    )
        ...
        with patch(
            ...
            with patch(
                ...
                with patch.object(real_draw, "text") as mock_text:
                    ...
                    assert any("..." in call for call in text_calls), (
                        "Truncated message with '...' should be drawn"
                    )
        ...
        with patch(
            ...
            with patch(
                ...
                with patch.object(real_draw, "text") as mock_text:
                    ...
                    text_calls = [str(call) for call in mock_text.call_args_list]
                    assert any("📱 Cached Data" in call for call in text_calls), (
                        "Cached data header should be drawn"
                    )

                    # Check for event time format
                    assert any("14:00 - 15:00" in call for call in text_calls), (
                        "Event time should be drawn"
                    )

                    # Check that bullet points are drawn for events
                    assert any("•" in call for call in text_calls), (
                        "Bullet points should be drawn for events"
                    )
            ...
            with patch(
                ...
                with patch.object(real_draw, "text") as mock_text:
                    ...
                    mock_text.assert_any_call(
                        ANY, "❌ No cached data available", fill=ANY, font=ANY
                    )
    ...
    def test_render_error_image_when_called_directly_then_returns_basic_error_image(
        ...
        with patch("calendarbot.display.epaper.integration.eink_whats_next_renderer.ImageFont"):
            temp_renderer = EInkWhatsNextRenderer(
                {"display": {"type": "epaper"}}, mock_renderer.display
            )
        ...
        with patch(
            ...
            with patch(
                ...
                with patch.object(real_draw, "text") as mock_text:
                    ...
                    text_calls = [str(call) for call in mock_text.call_args_list]
                    assert any("🔐" in call for call in text_calls), (
                        "Authentication icon should be drawn"
                    )
                    assert any("Authentication" in call for call in text_calls), (
                        "Authentication title should be drawn"
                    )
                    assert any(verification_uri in call for call in text_calls), (
                        "Verification URI should be drawn"
                    )
                    assert any(user_code in call for call in text_calls), (
                        "User code should be drawn"
                    )
        ...
        with patch(
            ...
            with patch(
                ...
                with patch.object(real_draw, "text") as mock_text:
                    ...
                    # Check for truncation without specifying exact coordinates
                    text_calls = [str(call) for call in mock_text.call_args_list]
                    assert any("microsoft.com" in call for call in text_calls), (
    ...
    def test_render_error_when_different_display_capabilities_then_adapts_rendering(
        ...
        mono_capabilities = MockDisplayCapabilities(supports_grayscale=False, supports_red=False)
        mono_display = MockDisplay(capabilities=mono_capabilities)

        # Test with grayscale display
        gray_capabilities = MockDisplayCapabilities(supports_grayscale=True, supports_red=False)
        gray_display = MockDisplay(capabilities=gray_capabilities)

        # Test with color display
        color_capabilities = MockDisplayCapabilities(supports_grayscale=True, supports_red=True)
                    ...
                    with patch(
                        ...
                        if display.capabilities.supports_red:
                            assert mock_new.call_args[0][0] == "RGB", (
                                "Should use RGB mode for color display"
                            )
                        elif display.capabilities.supports_grayscale:
                            assert mock_new.call_args[0][0] == "L", (
                                "Should use L mode for grayscale display"
                            )
                        else:
                            assert mock_new.call_args[0][0] == "1", (
                                "Should use 1 mode for monochrome display"
                            )
