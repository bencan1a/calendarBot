...
class TestEInkWhatsNextRendererSharedStyling:
    ...
    def test_init_when_shared_styling_available_then_uses_shared_styling(
        self, mock_settings: Dict[str, Any], mock_display: MockDisplay, mock_shared_styling: MagicMock
    ) -> None:
    ...
    def test_render_when_shared_styling_available_then_uses_shared_colors(
        ...
        mock_view_model_with_events: MockViewModel, mock_shared_styling: MagicMock,
        ...
        """
        # Mock the imports - note that we're patching the imported functions directly
        with patch("calendarbot.display.epaper.integration.eink_whats_next_renderer.get_colors_for_renderer", mock_get_colors_for_renderer):
            with patch("calendarbot.display.epaper.integration.eink_whats_next_renderer.get_typography_for_renderer") as mock_get_typography:
    ...
    def test_render_zone1_when_shared_styling_available_then_uses_shared_typography(
        self, mock_settings: Dict[str, Any], mock_display: MockDisplay, 
        mock_shared_styling: MagicMock, mock_get_typography_for_renderer: MagicMock
    ) -> None:
        ...
        # Mock the imports - note that we're patching the imported functions directly
        with patch("calendarbot.display.epaper.integration.eink_whats_next_renderer.get_colors_for_renderer") as mock_get_colors:
            with patch("calendarbot.display.epaper.integration.eink_whats_next_renderer.get_typography_for_renderer", mock_get_typography_for_renderer):
    ...
    def test_render_zone2_when_shared_styling_available_then_uses_shared_colors_and_typography(
        self, mock_settings: Dict[str, Any], mock_display: MockDisplay, 
        mock_shared_styling: MagicMock, mock_get_colors_for_renderer: MagicMock,
    ...
    def test_render_zone4_when_shared_styling_available_then_uses_shared_colors_and_typography(
        self, mock_settings: Dict[str, Any], mock_display: MockDisplay, 
        mock_view_model_with_events: MockViewModel, mock_shared_styling: MagicMock,
        mock_get_colors_for_renderer: MagicMock, mock_get_typography_for_renderer: MagicMock
    ) -> None:
        ...
        with patch("calendarbot.display.epaper.integration.eink_whats_next_renderer.get_colors_for_renderer", mock_get_colors_for_renderer):
            with patch("calendarbot.display.epaper.integration.eink_whats_next_renderer.get_typography_for_renderer", mock_get_typography_for_renderer):
                    ...
                    renderer._render_zone4_context(mock_draw, "L", cast(WhatsNextViewModel, mock_view_model_with_events), 0, 330, 300, 70)
    ...
    def test_render_full_image_when_shared_styling_available_then_uses_shared_layout(
        ...
        with patch("calendarbot.display.epaper.integration.eink_whats_next_renderer.get_colors_for_renderer") as mock_get_colors:
            with patch("calendarbot.display.epaper.integration.eink_whats_next_renderer.get_typography_for_renderer") as mock_get_typography:
                with patch("calendarbot.display.epaper.integration.eink_whats_next_renderer.get_layout_for_renderer", mock_get_layout_for_renderer):
                        # Mock the get_colors_for_renderer and get_typography_for_renderer functions
                        mock_get_colors.return_value = {
                            "background": 255,
                            "text_primary": 0
                        ...
                        result = renderer._render_full_image(cast(WhatsNextViewModel, mock_view_model_with_events))
    ...
    def test_visual_consistency_when_compared_to_web_then_matches_styling(
        ...
        """
        # Mock the imports - note that we're patching the imported functions directly
        with patch("calendarbot.display.shared_styling.SharedStylingConstants", mock_shared_styling):
            ...
            for key, value in css_colors.items():
                ...
                assert mock_shared_styling.COLORS[key].lower() == value.lower(), f"Color '{key}' should match CSS value"
            
            # Check that SharedStylingConstants typography matches CSS values
            for key, value in css_typography.items():
                assert key in mock_shared_styling.TYPOGRAPHY["html"], f"HTML typography should contain '{key}'"
                assert mock_shared_styling.TYPOGRAPHY["html"][key] == value, f"Typography '{key}' should match CSS value"
                
                # Check that PIL values are numeric equivalents
                pil_value = int(value.replace("px", ""))
                assert mock_shared_styling.TYPOGRAPHY["pil"][key] == pil_value, f"PIL typography '{key}' should be numeric equivalent of CSS value"