...
def mock_view_model_with_next_event() -> MockViewModel:
    ...
    return MockViewModel(next_events=[next_event])
...
def mock_view_model_with_current_event() -> MockViewModel:
    ...
    return MockViewModel(current_events=[current_event])
...
def mock_view_model_with_cached_data() -> MockViewModel:
    ...
    return MockViewModel(next_events=[next_event], is_cached=True)
...
class TestEInkWhatsNextRendererZones:
    ...
    def test_render_zone1_when_no_next_event_then_renders_plenty_of_time(
        ...
        mock_renderer._render_zone1_time_gap(
            draw=mock_draw, mode="L", next_event=None, x=0, y=0, width=300, height=130
        )

        # Verify the "Plenty of time" text was drawn
        text_calls = [
            call for call in mock_draw.text.call_args_list if "Plenty of time" in call[0][1]
        ]
        ...
        assert mock_renderer._draw_rounded_rectangle.call_count > 0, (
            "Should draw rounded rectangle container"
        )
    ...
    def test_render_zone1_when_has_next_event_then_renders_time_remaining(
        ...
        mock_renderer._render_zone1_time_gap(
            draw=mock_draw, mode="L", next_event=next_event, x=0, y=0, width=300, height=130
        )
    ...
    def test_render_zone1_when_different_time_values_then_formats_correctly(
        ...
        mock_renderer._render_zone1_time_gap(
            draw=mock_draw, mode="L", next_event=event_minutes, x=0, y=0, width=300, height=130
        )

        # Test with hours only (no remaining minutes)
        event_hours = MockEventData(time_until_minutes=120)
        mock_renderer._render_zone1_time_gap(
            draw=mock_draw, mode="L", next_event=event_hours, x=0, y=0, width=300, height=130
        )

        # Test with hours and minutes
        event_mixed = MockEventData(time_until_minutes=125)
        mock_renderer._render_zone1_time_gap(
            draw=mock_draw, mode="L", next_event=event_mixed, x=0, y=0, width=300, height=130
        )
        ...
        assert any("2 HOURS 5 MINUTES" in text for text in text_calls), (
    ...
    def test_render_zone2_when_no_next_event_then_renders_nothing(
        ...
        mock_renderer._render_zone2_meeting_card(
            draw=mock_draw, mode="L", next_event=None, x=0, y=130, width=300, height=200
        )

        # Verify no drawing methods were called
        assert not mock_draw.text.called, "Should not draw any text"
        assert mock_renderer._draw_rounded_rectangle.call_count == 0, (
            "Should not draw rounded rectangle"
        )
    ...
    def test_render_zone2_when_has_next_event_then_renders_event_details(
        ...
        # Call the zone rendering method with next event
        mock_renderer._render_zone2_meeting_card(
            draw=mock_draw, mode="L", next_event=next_event, x=0, y=130, width=300, height=200
        )
        ...
        assert any(next_event.formatted_time_range in text for text in text_calls), (
            "Should draw time range"
        )
    ...
    def test_render_zone2_when_long_text_then_truncates_properly(
        self,
        mock_renderer: EInkWhatsNextRenderer,
        mock_view_model_with_long_event_data: MockViewModel,
    ) -> None:
        ...
        assert any(truncated_location in text for text in text_calls), (
            "Should truncate long location"
        )
    ...
    def test_render_zone4_when_no_next_event_then_renders_focus_message(
        ...
        text_calls = [
            ...
            if "Plenty of time to focus" in call[0][1]
        ]
    ...
    def test_render_zones_when_exception_occurs_then_handles_gracefully(
        ...
        mock_renderer._render_zone1_time_gap = MagicMock(
            side_effect=ValueError("Zone 1 rendering error")
        )
