...
class ICSConfig(BaseModel):
    ...
    enable_caching: bool = Field(default=True, description="Enable HTTP caching with ETag/Last-Modified")
    
    # Parsing settings
    filter_busy_only: bool = Field(default=True, description="Only show busy/tentative events")
    expand_recurring: bool = Field(default=False, description="Expand recurring events (experimental)")
