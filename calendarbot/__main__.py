...
def parse_components(components_str: str) -> List[str]:
    ...
    valid_components = {"sources", "cache", "display"}
...
Examples:
  ...
  %(prog)s --rpi --web               # Run in RPI e-ink mode with web interface
    ...
    # Test mode arguments
    parser.add_argument(
        "--test-mode", action="store_true", help="Run validation tests instead of daemon mode"
    )
    ...
    parser.add_argument("--no-cache", action="store_true", help="Skip using cached data in tests")

    parser.add_argument(
        "--components",
        type=parse_components,
        default=["sources", "cache", "display"],
        help="Comma-separated components to test: sources,cache,display (default: all)",
    )
    ...
    parser.add_argument(
        "--host",
        default=None,
        help="Host for web server (default: auto-detect local network interface, use 0.0.0.0 for all interfaces)",
    )
...
async def run_web_mode(args) -> int:
    ...
    try:
        ...
        # Override web settings from command line with smart network detection
        logger.debug(
            f"Original settings - host: {updated_settings.web_host}, port: {updated_settings.web_port}"
        )

        # Import network utilities for smart host detection
        from calendarbot.utils.network import get_local_network_interface, validate_host_binding

        # Use smart network detection if host not explicitly specified
        if args.host is None:
            detected_host = get_local_network_interface()
            updated_settings.web_host = detected_host
            logger.info(f"Auto-detected network interface: {detected_host}")
        else:
            # Validate and warn about explicit host binding
            validated_host = validate_host_binding(args.host, warn_on_all_interfaces=True)
            updated_settings.web_host = validated_host
            logger.info(f"Using explicit host binding: {validated_host}")
