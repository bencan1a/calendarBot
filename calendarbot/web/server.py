...
class WebRequestHandler(BaseHTTPRequestHandler):
    ...
    def _handle_theme_api(self, params):
        ...
        theme = params.get('theme', [''])[0] if isinstance(params, dict) and 'theme' in params else params.get('theme', '')
    ...
    def _send_response(self, status_code: int, content: str | bytes, content_type: str, binary: bool = False):
        """Send HTTP response."""
...
class WebServer:
    ...
    def get_calendar_html(self) -> str:
        ...
        try:
            ...
            if self.navigation_state:
                ...
                logger.debug(f"Interactive mode - getting events for {selected_date} ({start_datetime} to {end_datetime})")
                
                # This should be async, but we're in sync context
                # In a real implementation, we'd need to handle this properly
                events = asyncio.run(self.cache_manager.get_events_by_date_range(start_datetime, end_datetime))
                ...
                logger.debug(f"Non-interactive mode - getting events for today {today} ({start_datetime} to {end_datetime})")
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_date": self.navigation_state.selected_date.isoformat() if self.navigation_state else date.today().isoformat()
        }
