...
function setupCompactEInkEventHandling() {
    ...
    // Handle keyboard events for compact e-ink
    const setupCompactKeyboardHandling = () => {
        document.addEventListener('keydown', (event) => {
            // Handle layout switching with L key
            if (event.key === 'l' || event.key === 'L') {
                event.preventDefault();
                event.stopPropagation();
                console.log('Compact layout switch (L) key pressed');
                cycleCompactLayout();
            }
            // Handle theme switching with T key
                ...
                event.preventDefault();
                event.stopPropagation();
                console.log('Compact theme toggle (T) key pressed');
                toggleCompactTheme();
            }
            // Handle refresh with R key
                ...
                event.preventDefault();
                event.stopPropagation();
                console.log('Compact refresh (R) key pressed');
                refreshCompact();
            }
        }, { passive: false });
    };
    ...
    // Setup keyboard handling for compact e-ink
    setupCompactKeyboardHandling();
...
/**
 * Cycle through available layouts for compact display
 */
async function cycleCompactLayout() {
    console.log('Compact E-ink layout cycle requested');

    try {
        showCompactLoadingIndicator('Switching layout...');

        const response = await fetch(EInkCompactCalendar.config.endpoints.layout, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        if (data.success) {
            console.log(`Compact layout changed to: ${data.layout}`);
            
            // For layout changes, we need to reload the page to get new CSS/JS
            // This ensures the correct layout-specific resources are loaded
            showCompactSuccessMessage(`Layout: ${data.layout}`);
            ...
            console.error('Compact layout cycle failed:', data.error);
            showCompactErrorMessage('Layout switch failed');
        }

    } catch (error) {
        console.error('Compact layout cycle error:', error);
        showCompactErrorMessage('Layout switch error');
    } finally {
        hideCompactLoadingIndicator();
    }
}
...
/**
 * Set specific layout for compact display
 * @param {string} layout - Layout name to set
 */
async function setCompactLayout(layout) {
    console.log(`Setting compact layout to: ${layout}`);
    ...
    try {
        ...
        if (data.success) {
            console.log(`Compact layout set to: ${data.layout}`);
            ...
            console.error('Compact layout set failed:', data.error);
            showCompactErrorMessage('Layout switch failed');
        }

    } catch (error) {
        console.error('Compact layout set error:', error);
        showCompactErrorMessage('Layout switch error');
    } finally {
        hideCompactLoadingIndicator();
    }
...
window.cycleLayout = cycleCompactLayout;
window.setLayout = setCompactLayout;
...
window.calendarBot = {
    ...
    cycleLayout: cycleCompactLayout,
    setLayout: setCompactLayout,
