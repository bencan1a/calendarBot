...
from .models import (
    CalendarEvent, DateTimeInfo, Location, EventStatus,
    Attendee, AttendeeType, ResponseStatus
)
...
class ICSParser:
    ...
    def parse_ics_content(self, ics_content: str) -> ICSParseResult:
        ...
        try:
            ...
            logger.info(f"Parsed {len(filtered_events)} events from ICS content "
    ...
    def _parse_event_component(self, component: ICalEvent, 
                             default_timezone: Optional[str] = None) -> Optional[CalendarEvent]:
        ...
        try:
            ...
            if description:
                ...
                if any(indicator in desc_str for indicator in ['teams.microsoft.com', 'zoom.us', 'meet.google.com']):
    ...
    def expand_recurring_events(self, events: List[CalendarEvent], 
