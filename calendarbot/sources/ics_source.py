...
from ..ics import ICSFetcher, ICSParser, ICSSource, ICSAuth, AuthType
from ..ics.exceptions import ICSError, ICSFetchError, ICSParseError, ICSAuthError, ICSNetworkError
from .models import SourceConfig, SourceHealthCheck, SourceMetrics, SourceStatus
from .exceptions import SourceError, SourceConnectionError, SourceDataError
...
class ICSSourceHandler:
    ...
    def __init__(self, config: SourceConfig, settings):
        ...
        if self.config.auth_type:
            if self.config.auth_type.lower() == 'basic':
                ...
                auth = ICSAuth(
                    ...
                    bearer_token=self.config.auth_config.get('token')
                )
    ...
    async def fetch_events(self, use_cache: bool = True) -> List[CalendarEvent]:
        ...
        try:
            ...
            async with self.fetcher as fetcher:
                ...
                logger.info(f"Successfully fetched {len(parse_result.events)} events from {self.config.name}")
    ...
    async def get_events_for_date_range(self, start_date: datetime, 
