...
class CalendarServiceTemplate:
    ...
    def __init__(self, name: str, description: str, url_pattern: str, 
                 auth_type: str = "none", instructions: str = ""):
        self.name = name
...
3. If authentication is required, you'll configure that next
    ...
    def get_input(self, prompt: str, default: Optional[str] = None, 
                  required: bool = True, validate_func=None) -> str:
    ...
    def configure_ics_url(self, service_key: str) -> Dict[str, Any]:
        ...
        return {
            'url': url,
            'recommended_auth': template.auth_type
        }
    ...
    def configure_advanced_settings(self) -> Dict[str, Any]:
        ...
        log_level = self.get_choice(
            "Choose default logging level:",
            log_levels
        )
    ...
    def generate_config_content(self, ics_config: Dict[str, Any], 
                              advanced_settings: Dict[str, Any]) -> str:
        ...
        yaml_content += f"# Generated by setup wizard on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    def save_configuration(self, config_content: str) -> Path:
        ...
        choice = self.get_choice(
            ...
            location_names,
            [str(path) for path in location_paths]
        )
        ...
        if config_path.exists():
            if not self.get_yes_no(f"Configuration file already exists at {config_path}. Overwrite?"):
...
def run_simple_wizard() -> bool:
    ...
    try:
        ...
        print("For full setup options, use: python -c 'import asyncio; from calendarbot.setup_wizard import run_setup_wizard; asyncio.run(run_setup_wizard())'")
        print()
