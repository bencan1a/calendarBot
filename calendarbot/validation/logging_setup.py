...
class ValidationFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        record.msecs_formatted = f"{record.msecs:03.0f}"
        
        # Extract component from logger name if possible
...
class ComponentFilter(logging.Filter):
    ...
    def __init__(self, allowed_components: Optional[list] = None):
        ...
        """
        super().__init__()
        self.allowed_components = set(allowed_components) if allowed_components else None
...
def setup_validation_logging(verbose: bool = False, 
                           components: Optional[list] = None,
                           log_file: Optional[str] = None) -> Dict[str, logging.Logger]:
    ...
    component_loggers = {}
    component_names = ['auth', 'api', 'cache', 'display', 'validation']
...
def get_validation_logger(component: str) -> logging.Logger:
    ...
    Args:
        component: Component name (auth, api, cache, display, validation)
...
def log_validation_result(logger: logging.Logger, test_name: str, success: bool, 
                         message: str, duration_ms: Optional[int] = None):
