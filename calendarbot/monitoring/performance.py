...
class PerformanceLogger:
    ...
    def _setup_metrics_logger(self) -> logging.Logger:
        ...
        if self.settings and hasattr(self.settings, 'data_dir'):
            ...
            metrics_dir = Path.home() / ".local" / "share" / "calendarbot" / "performance" / "metrics"
    ...
    def start_timer(self, operation: str, component: str = "", 
    ...
    def stop_timer(self, timer_id: str, component: str = "", 
                  operation: str = "", correlation_id: Optional[str] = None,
    ...
    def log_request_performance(self, method: str, url: str, 
    ...
    def log_cache_performance(self, cache_name: str, 
                            hits: int, misses: int, 
                            total_requests: int,
                            component: str = "cache",
                            correlation_id: Optional[str] = None):
    ...
    def log_database_performance(self, query_type: str, 
    ...
    def _check_thresholds(self, metric: PerformanceMetric):
        ...
        if metric.metric_type == MetricType.REQUEST and 'duration' in metric.name:
            ...
            if duration > self.thresholds['request_duration_error']:
                self.logger.error(f"Request duration critical: {duration:.2f}s exceeds {self.thresholds['request_duration_error']}s threshold")
            elif duration > self.thresholds['request_duration_warning']:
                self.logger.warning(f"Request duration high: {duration:.2f}s exceeds {self.thresholds['request_duration_warning']}s threshold")
            ...
            if memory_mb > self.thresholds['memory_usage_error']:
                self.logger.error(f"Memory usage critical: {memory_mb:.1f}MB exceeds {self.thresholds['memory_usage_error']}MB threshold")
            elif memory_mb > self.thresholds['memory_usage_warning']:
                self.logger.warning(f"Memory usage high: {memory_mb:.1f}MB exceeds {self.thresholds['memory_usage_warning']}MB threshold")
            ...
            if miss_rate > self.thresholds['cache_miss_rate_error']:
                self.logger.error(f"Cache miss rate critical: {miss_rate:.1%} exceeds {self.thresholds['cache_miss_rate_error']:.1%} threshold")
            elif miss_rate > self.thresholds['cache_miss_rate_warning']:
                self.logger.warning(f"Cache miss rate high: {miss_rate:.1%} exceeds {self.thresholds['cache_miss_rate_warning']:.1%} threshold")
    ...
    def get_performance_summary(self, hours: int = 1) -> Dict[str, Any]:
        ...
        for metric_type in MetricType:
            ...
            if type_metrics:
                by_type[metric_type.value] = {
                    ...
                    'avg_value': sum(float(m.value) for m in type_metrics if isinstance(m.value, (int, float))) / len(type_metrics)
                }
        ...
        if request_metrics:
            durations = [float(m.value) for m in request_metrics if isinstance(m.value, (int, float))]
...
class PerformanceLoggerMixin:
    ...
    def stop_performance_timer(self, timer_id: str, operation: str = "", 
                             correlation_id: Optional[str] = None,
                             metadata: Optional[Dict[str, Any]] = None) -> float:
        """Stop a performance timer for this component."""
        component = self.__class__.__name__
        return self._perf_logger.stop_timer(timer_id, component, operation, correlation_id, metadata)
    
    def log_performance_metric(self, name: str, value: Union[float, int], 
                             metric_type: MetricType = MetricType.GAUGE,
...
@contextmanager
def performance_timer(operation: str, component: str = "", 
                     correlation_id: Optional[str] = None,
...
@contextmanager
def memory_monitor(component: str = "system", operation: str = "memory_check",
                  correlation_id: Optional[str] = None,
                  logger: Optional[PerformanceLogger] = None):
    """
...
@contextmanager
def cache_monitor(cache_name: str, component: str = "cache",
                 correlation_id: Optional[str] = None,
                 logger: Optional[PerformanceLogger] = None):
    """
    ...
    try:
        ...
        if monitor.total_requests > 0:
            perf_logger.log_cache_performance(
                cache_name, monitor.hits, monitor.misses, 
                monitor.total_requests, component, correlation_id
            )
