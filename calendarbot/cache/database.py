...
class DatabaseManager:
    ...
    async def store_events(self, events: List[CachedEvent]) -> bool:
        ...
        try:
            ...
            async with aiosqlite.connect(str(self.database_path)) as db:
                ...
                """, [
                    (
                        event.id, event.graph_id, event.subject, event.body_preview,
                        event.start_datetime, event.end_datetime, 
                        event.start_timezone, event.end_timezone,
                        event.is_all_day, event.show_as, event.is_cancelled, event.is_organizer,
                        event.location_display_name, event.location_address,
                        event.is_online_meeting, event.online_meeting_url, event.web_link,
                        event.is_recurring, event.series_master_id, 
                        event.cached_at, event.last_modified
                    ) for event in events
                ])
    ...
    async def get_events_by_date_range(self, start_date: datetime,
        ...
        try:
            ...
            logger.info(f"DEBUG: Query logic - events WHERE start_datetime <= {end_str} AND end_datetime >= {start_str}")
    ...
    async def get_database_info(self) -> Dict[str, Any]:
        ...
        try:
            async with aiosqlite.connect(str(self.database_path)) as db:
                ...
                """)
                rows = await cursor.fetchall()
                info['events_by_date'] = [dict(row) for row in rows]
