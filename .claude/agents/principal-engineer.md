...
- Avoid abstract factory patterns for 2 implementations
- Avoid heavy ORM frameworks for simple queries
...
**Examples**:
- Use `yield` for event streams
- LRU cache with maxsize for repeated queries
- Stream processing for large ICS files
- Avoid loading entire calendar into memory
- Avoid unbounded caches or data structures
...
# Blocking I/O in async context - AVOID
...
- **Email/SMS**: Requires external service (SendGrid, Twilio), adds complexity
...
- **Con**: More files, slightly higher complexity
- **Con**: Requires updating imports across codebase
